...
import net.marsbytes.app.recruitment.repository.StepRepository;
...
class StepResourceIT {
    ...
    public static Step createEntity(EntityManager em) {
        Step step = new Step().code(DEFAULT_CODE).name(DEFAULT_NAME);
        return step;
    }
    ...
    public static Step createUpdatedEntity(EntityManager em) {
        Step step = new Step().code(UPDATED_CODE).name(UPDATED_NAME);
        return step;
    }
    ...
    void createStep() throws Exception {
        ...
        restStepMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(step))
            )
    ...
    void putNewStep() throws Exception {
        ...
        updatedStep.code(UPDATED_CODE).name(UPDATED_NAME);
    ...
    void putWithMissingIdPathParamStep() throws Exception {
        ...
        restStepMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(step))
            )
    ...
    void fullUpdateStepWithPatch() throws Exception {
        ...
        partialUpdatedStep.code(UPDATED_CODE).name(UPDATED_NAME);
    ...
    void deleteStep() throws Exception {
        ...
        // Delete the step
        restStepMockMvc
            .perform(delete(ENTITY_API_URL_ID, step.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
