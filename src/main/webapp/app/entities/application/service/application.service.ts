...
export class ApplicationService {
  ...
  protected convertDateFromClient(application: IApplication): IApplication {
    return Object.assign({}, application, {
      dateOfApplication: application.dateOfApplication?.isValid() ? application.dateOfApplication.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateOfApplication = res.body.dateOfApplication ? dayjs(res.body.dateOfApplication) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((application: IApplication) => {
        application.dateOfApplication = application.dateOfApplication ? dayjs(application.dateOfApplication) : undefined;
      });
    }
    return res;
  }
